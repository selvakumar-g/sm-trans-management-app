UTIL = function() {
			...
			validateForm = function() {
				...
				$('form')
						...
						.each(
								function(idx, node) {
									if ($(this).data('rules')) {
										var rules = JSON.parse($(this).data(
												'rules').replace(/'/g, '"'));
										$(rules)
												.each(
														function(idx, rule) {
			...
			clearFormValidation = function() {
				$('form').find('.error-container').each(
						function(idx, node) {
							$(node).removeClass('error-container').children(
									'.error-text').empty();
						});
			},
			...
			shutdown = function() {
				toastMessage('Shutdown in progress .... !!')
